...
#if defined(__MSP430_HAS_USCI__) && defined(_OLD_)
SPI::SPI()
{
	/* USI SPI setup */
	USICTL0 |= USISWRST;
...
uint16_t SPI::transfer9(uint16_t inw)
{
	USICTL1 |= USIIE;
	USISR = inw;
...
SPI::SPI()
{
	...
	UCB0CTL1 |= UCSWRST;
	UCB0MCTL = 0x00;  // Clearing modulation control per TI user's guide recommendation
...
*	P1REN = p1ren_save;
*
*	retw |= transfer( (uint8_t)(inw & 0x00FF) );
*	return retw;
*}
*/
#endif
...
#if defined(__MSP430_HAS_USCI__) && defined(SPI_DRIVER_USCI_PROVIDE_ISR)
#pragma vector = USCIAB0RX_VECTOR
__interrupt void USCI_RX(void) {

	#ifdef SPI_DRIVER_USCI_A
	IE2 &= ~UCA0RXIE;
	#endif

	#ifdef SPI_DRIVER_USCI_B
	IE2 &= ~UCB0RXIE;
	#endif
...
#if defined(__MSP430_HAS_USCI_A0__) && defined(SPI_DRIVER_USCI_PROVIDE_ISR) && defined(SPI_DRIVER_USCI_A)
...
#if defined(__MSP430_HAS_USCI_B0__) && defined(SPI_DRIVER_USCI_PROVIDE_ISR) && defined(SPI_DRIVER_USCI_B)
